...
from neurologic_astar.topK import topk_huggingface, ConstrainedHypothesis
...
class BeamHypotheses(object):
    def __init__(self, num_beams, max_length, length_penalty, early_stopping, lambd: float, gamma: float, new_line_token: int, loop_tokens: list):
        """
...
def _generate_beam_search(
    ...
    start_idx = (last_non_masked_idx).view(-1, 1).repeat(1, model.config.vocab_size).unsqueeze(1).long()
    ...
    generated_hyps = [
        BeamHypotheses(num_beams, max_length, length_penalty, early_stopping=early_stopping, lambd=lambd, gamma=gamma, new_line_token=new_line_token, loop_tokens=loop_tokens)
    ...
    while cur_len < max_length:
        ...
        if _use_cache(model, outputs, use_cache):
            #past = outputs[1]
        ...
        if do_sample:
            ...
            assert _use_cache(model, outputs, use_cache) and use_cache, 'model not using past'
            temp_attention_mask = attention_mask if model.config.is_encoder_decoder else \
