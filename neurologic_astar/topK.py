...
from neurologic_astar.lexical_constraints import ConstrainedHypothesis, ConstrainedCandidate
from neurologic_astar.look_ahead import _generate_beam_search, _generate_greedy, _generate_sample
...
def topk_huggingface(timestep: int,
    ...
    for sentno in range(batch_size):
        ...
        idxs = torch.arange(sentno * beam_size, sentno * beam_size + beam_size, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))#device=temp_past[0].device)
        ...
        select_best_ids[sentno], select_best_word_ids[sentno], select_seq_scores[sentno],\
            select_hypotheses[sentno], select_num_mets[sentno] = _sequential_topk(sentno,
                                                                                  ...
                                                                                  model._reorder_cache(temp_past, idxs),
                                                                                  #temp_past,
                                                                                  decode_kwargs,
                                                                                  model_specific_kwargs)
...
def _sequential_topk(sentno: int,
    ...
    if candidates:
        ...
        for start in range(0, len(all_sorted_candidates), chunk_size):
            ...
            past = model._reorder_cache(temp_past, back_ptrs)
            #past = temp_past
            ...
            if look_ahead_scores.shape[0]:
                if look_ahead_sample:
                    future_state = _generate_sample(model, input_ids, look_ahead_scores, timestep + 1,
                                                    ...
                                                    position_ids, model._reorder_cache, **decode_kwargs)
                else:
